...
def users_df(section=1):
    ...
    try:
        filter_query = f"select unique_mem_id from user_demographic order by unique_mem_id asc limit 10000"
    ...
    for num, user in enumerate(users_sections[section - 1]):
        try:
            filter_query = f"(select {', '.join(field for field in fields)} from card_record where unique_mem_id = '{user[0]}') union (select {', '.join(field for field in fields)} from bank_record where unique_mem_id = '{user[0]}')"
            transaction_query = execute_read_query(connection, filter_query)
            df = pd.DataFrame(transaction_query, columns=fields)
            print(f"{len(df)} transactions for user {user[0]}, {num+1}/10000 users, {round(((num+1)/10000)*100, 2)}%.")
        except OperationalError as e:
            print(f"The error '{e}' occurred")
            connection.rollback
    ...
    results = {'avr_income_pu': avr_inc_pu.mean(),
