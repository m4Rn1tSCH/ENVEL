...
'''
            Tensorflow Rework
'''
#pass all feature names as list(needs to be string for feat_col)
feature_names = list(bank_df.columns.values)
#specify input to the model; all numeric values with list comprehension
...
#%%
#OPTIONAL STEP WITH CUSTOM ESTIMATOR
...
classifier = tf.estimator.DNNClassifier(
    ...
    n_classes=3,
    # Path to where checkpoints etc are stored; current working directory
    model_dir=os.getcwd())
...
#%%
'''
            Recurrent Neural Network setup TF
'''
# import dataset
dataset = bank_df
df_test = dataset.copy()
df_test = df_test.dropna()
df_test = df_test[df_test.columns.values]
#%%
...
df_test_norm = normalize_data(df=df_test_norm)
