...
def db_insert_section(section=1):
    ...
    for num, user in enumerate(main_df.groupby('unique_mem_id')):
        ...
        try:
            ...
            for index, i in enumerate(prediction_df['transaction_category_name']):
                if i in bank_ess:
            ...
            prediction_df = prediction_df.assign(tr_essentiality=tr_class.values)
        ...
        try:
            ...
            prediction_df = prediction_df.assign(envelope_category=envel_cat.values)
        except BaseException as error:
